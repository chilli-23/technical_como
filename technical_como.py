...
def load_master_data():
    ...
    """
    # This query now joins the 'alarm' table using the 'alarm_standard' column as the key.
    # It still selects the equipment_tag_id for display purposes.
    query = text("""
        SELECT 
            d.date,
            d.equipment_name,
...
def main():
    ...
    # --- NEW: Three-Level Cascading Filters ---
    col1, col2, col3 = st.columns(3)

    # --- Filter 1: Equipment Name ---
    with col1:
    ...
    # --- Filter 2: Technology (depends on Equipment) ---
    with col2:
        if selected_equipment:
            # Filter the dataframe by the selected equipment to find available technologies
    ...
    # --- Filter 3: Point Measurement (depends on Equipment AND Technology) ---
    with col3:
        if selected_equipment and selected_technology:
            # Filter by both equipment and technology to find the final list of points
            point_df = df[
            ...
            selected_points = st.multiselect(
                label="3. Select Measurement Point(s)",
                options=point_list,
    ...
    if selected_equipment and selected_technology and selected_points:
        ...
        # The final filtered DataFrame for display
        filtered_df = df[
            (df['equipment_name'] == selected_equipment) &
            (df['technology'] == selected_technology) &
            (df['point_measurement'].isin(selected_points))
        ].sort_values(by='date')

        if not filtered_df.empty:
            # --- Trend Chart ---
            filtered_df['legend_label'] = filtered_df['point_measurement'] + ' (' + filtered_df['unit'].astype(str) + ')'
            chart_title = f"Vibration Trend for: {selected_equipment}"
            st.subheader(chart_title)
            
            fig = px.line(
                filtered_df, x='date', y='value', color='legend_label', markers=True,
            ...
            alarm_standards_df.rename(columns={
                'point_measurement': 'Point Measurement', 'equipment_tag_id': 'Tag ID', 'alarm_standard': 'Alarm Standard', 'excellent': 'Excellent',
                'acceptable': 'Acceptable', 'requires_evaluation': 'Requires Evaluation', 'unacceptable': 'Unacceptable', 'unit': 'Unit'
            }, inplace=True)
            ...
            column_styles = {
                'Excellent': 'background-color: rgba(0, 100, 0, 0.6)', 'Acceptable': 'background-color: rgba(144, 238, 144, 0.6)',
                'Requires Evaluation': 'background-color: rgba(255, 255, 0, 0.6)', 'Unacceptable': 'background-color: rgba(255, 0, 0, 0.6)'
            }
            ...
            for point in selected_points:
                ...
                table_df_to_display.rename(columns={
                    ...
                    'point_measurement': 'Point Measurement', 'equipment_tag_id': 'Tag ID', 'value': 'Value', 'unit': 'Unit', 'status': 'Status', 'note': 'Note'
                }, inplace=True)
