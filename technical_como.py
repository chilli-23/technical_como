    ...
    if st.button("3. Upload and Add Data"):
        if uploaded_file is not None and target_table is not None:
            try:
                ...
                # --- THIS IS THE CHANGE: Auto-clean rows with blank identifiers ---
                # This check only applies if the target table has an 'identifier' column
                if 'identifier' in upload_df.columns:
                    initial_row_count = len(upload_df)
                    # Drop rows where the 'identifier' column is NaN or None
                ...
                unique_key_map = {'data': 'identifier', 'alarm_standards': 'standard'}
